...
The market sells static software. We sell a **living system**. An n8n workflow breaks the moment the restaurant's operation shifts. An agent with the right capability adapts. That's defensible because building it well requires deep hospitality domain knowledge — and we're accumulating it client by client.

We're not selling automation. We're selling an **AI employee that knows the business**.

## The product — a modular AI employee

Starts with one capability. Learns the client's operation. Grows over time. Every new capability we add to the library deploys to existing clients as an upsell.

### The capability stack

- **Voice** — reservations, cancellations, upsells (Vapi + Twilio).
- **Email** — inquiry triage, group bookings, supplier comms (n8n today; replacing with a custom email-capability).
- **Social media** — content, scheduling, engagement.
- **Reviews** — Google / TripAdvisor monitoring + response drafting.
- **Dashboard** — visibility layer across all capabilities deployed to a client.

The engineering team doesn't specialize *per capability type*. **Skilled Builder builds reusable capabilities for the library. Implementation Engineer deploys any capability from the library to any client. Product Manager audits the library when a new client proposal arrives — reuse first, build only where the library doesn't cover.** Build once; deploy N.

## The moat

**Not the tech — the hospitality-specific capability library we're accumulating.** By client 20 we have 10–15 battle-tested capabilities no generalist agency can replicate. We already know the industry, the tools, the decision-makers.
...
- Our email capability: **3,700 SEK/month**.
...
- **Akira:** 20,000–57,000 SEK setup + 3,700 SEK/month per capability. Deployed in two weeks. ROI visible in month one. We sell employees.
...
- **Year 2 (2027):** Add Denmark. Capability library mature — deployment is near-fully templated.
...
- **~110,000 SEK / month MRR** at full Y1 run rate (30 × 3,700 SEK avg, single-capability per client dominant).
- **~970,000 SEK** in cumulative setup revenue.
- **~2.1M SEK total revenue** (MRR ramp + setups).
- **Year 1 is beachhead.** Validate the capability library, the demand-creation loop, the delivery pipeline. No rush on revenue — rush on proof.

### North Star (Y2 exit — end of 2027) — Scaling

- **1,000,000 SEK / month revenue run rate** (~12M SEK / year).
- Likely composition: ~150–200 clients × 2–3 capabilities per client on average. Multi-capability per client is where the library compounds.
- Denmark operational.
- Customer Success function detecting upsells from new capabilities automatically.
- Sales + Customer Success are the only human functions. Every other function is agent-supported.

Gap between Y1 and North Star is ~10×. Closed through: more clients × more capabilities per client × more geographies. Not through a hand-rolled pivot.
...
- **Denmark** operational.
- **Capability library matures** — deployment is near-fully templated.
- **Customer Success-led upsell flywheel** — every new capability auto-triggers a sweep for eligible existing clients.
...
2. **Library compounding** (Engineering team + PM + CS) — every capability built deploys to every eligible client. Build once, deploy many. PM audits fit when a proposal arrives; Skilled Builder builds the gap; Implementation Engineer deploys; CS sweeps existing clients on every new capability.
3. **Retention compounding** (Customer Success) — every retained client is a multi-capability expansion candidate + referral source.
