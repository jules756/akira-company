...
## 1b. Weekly audit-log review (Fridays)

If today is Friday, perform the weekly audit-log scan:

1. `GET /api/companies/{companyId}/activity?since=<7 days ago>&limit=500` — pull the last 7 days of mutations.
2. Scan for:
   - Unexpected actions (an agent doing something outside its allow-list per NOTION-PROTOCOL or GOVERNANCE).
   - Budget warnings (80% threshold crossings).
   - Approval-gate rejections (review each — is the gate right or too aggressive?).
   - Efficiency outliers (an agent spending many runs on a single issue — is it stuck?).
3. Post findings to your standing "Governance Review" issue (create on day 1 if missing), assigned to yourself with Jules cc'd via comment.
4. If any finding warrants a governance-phase regression (tighten a gate that was loosened prematurely), flag to Jules with a specific recommendation.
