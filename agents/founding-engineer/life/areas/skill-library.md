...
1. Founding Engineer ships skill X to `production`.
...
This compounds. Client 20 with 12 library skills is very different economics from client 20 with 3 library skills.
...
When Implementation Engineer or Customer Success flags an issue with an existing library skill (accent handling, integration breaking, performance drop), log here with the date, issue, fix shipped, and date shipped. Keeps skill evolution auditable.
